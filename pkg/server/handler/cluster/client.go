...
func (c *Client) GetConsoleOutput(ctx context.Context, organizationID, projectID, clusterID, machineID string, request *regionapi.ConsoleOutputRequest) (*regionapi.ConsoleOutputResponse, error) {
	namespace, err := common.New(c.client).ProjectNamespace(ctx, organizationID, projectID)
	if err != nil {
		return nil, err
	}
	...
	// REVIEW_ME: This conversion is needed because the region spec does not use $ref in the request body definition, which generated two different structs for the same data structure.
	getConsoleOutputRequest := regionapi.GetApiV1OrganizationsOrganizationIDProjectsProjectIDIdentitiesIdentityIDServersServerIDConsoleoutputJSONRequestBody{
		Lines: request.Lines,
	}

	resp, err := c.region.GetConsoleOutput(ctx, organizationID, projectID, cluster.Annotations[constants.IdentityAnnotation], machineID, getConsoleOutputRequest)
	if err != nil {
		// REVIEW_ME: Is there a way to check if the underlying error is a 404 not found?
