...
// aclWithOrgScopeCreate grants compute:clusters/Create at organization scope
// (no project-level entry), so CreateV2 must verify the project via the
// identity API.
...
// minimalClusterV2CreateRequest returns a ClusterV2Create request body with
// the given organization and project IDs.
func minimalClusterV2CreateRequest(orgID, projID string) *computeapi.ClusterV2Create {
	return &computeapi.ClusterV2Create{
		Metadata: coreapi.ResourceWriteMetadata{
			Name: "test-cluster",
		},
		Spec: computeapi.ClusterV2CreateSpec{
...
func TestCreateV2RBACOrgScopedProjectNotFound(t *testing.T) {
	...
	c := cluster.NewClient(nil, "", nil, mockIdentity, nil)

	ctx := rbac.NewContext(t.Context(), aclWithOrgScopeCreate())

	_, err := c.CreateV2(ctx, minimalClusterV2CreateRequest(organizationID, "nonexistent-project"))
...
func TestCreateV2RBACNoPermissions(t *testing.T) {
	...
	_, err := c.CreateV2(ctx, minimalClusterV2CreateRequest(organizationID, projectID))
